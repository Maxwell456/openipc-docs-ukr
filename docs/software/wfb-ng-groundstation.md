...
**6. Конфіг** — відредагуйте `/etc/wifibroadcast.cfg` (канал/регіон/потоки) і покладіть `/etc/gs.key`. Детально про параметри та генерацію ключів — на сторінці [Конфігурація](/software/wfb-ng-config).
