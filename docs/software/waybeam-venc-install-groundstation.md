...
Ця інструкція описує, як повністю замінити **Majestic** на **Waybeam** у зв'язці з **WFB-ng** (WiFi Broadcast) для FPV-стрімінгу з мінімальною затримкою.
...
При використанні `unix://` та `audioPort: 0` аудіо передається разом з відео через той самий канал. При `audioPort > 0` аудіо відправляється на окремий UDP-порт `127.0.0.1:<audioPort>`. Параметри `audio.*` (крім `audio.mute`) — restart-required; `audio.mute` змінюється на льоту.
:::
...
::: info
Формат MPEG-TS не потребує фіналізації файлу. Навіть при раптовому відключенні живлення запис буде читабельним до останнього записаного пакету. Файли ротуються на межах IDR за часом (`maxSeconds`) або розміром (`maxMB`).
:::
