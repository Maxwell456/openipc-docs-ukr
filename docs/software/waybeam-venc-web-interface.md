...
Waybeam включає вбудовану веб-панель та повний HTTP API для керування параметрами в реальному часі. Веб-панель доступна за адресою `http://<ip-камери>/` (порт за замовчуванням — 80) або просто `http://waybeam.local` — камера анонсує себе через mDNS. Довідник звірено з версією **v0.24.1** (липень 2026).
...
Поля конфігурації згруповані за **13 секціями**:
...
Значення `contract_version` / `config_schema_version` зростають з релізами (наприклад, у v0.19 контракт піднято до 0.11.0 через видалення `video0.frameLost`).
...
::: warning Індекси sensor.mode перенумеровано (v0.21 / v0.23)
Лінійки режимів на Maruko перероблено: IMX415 отримав non-binned 16:9 режими на 1485 Mbps (аж до `1920×1080@100`), IMX335 — лінійку «найкращий на кожен fps» (30/50/60/90/100) та ультранизьколатентний `1536×864@144` (v0.24). Індекси `sensor.mode` при цьому **перенумеровано** — після оновлення звіряйте старі конфіги зі списком `/api/v1/modes` або ставте `-1`.
:::
...
**Стабілізація** (`stab` / `stab-fill`, тільки Star6E) використовує Kalman-фільтр траєкторії; на Maruko відповідні контролі у WebUI неактивні — у чипа немає IVE-блоку. Тонке налаштування (всі restart; повторний вибір пресету скидає їх до дефолтів, тому **спочатку задавайте `framing`, потім переоприділення**):
