...
description: "Adaptive-Link в OpenIPC автоматично змінює бітрейт і потужність за сигналом. Встановлення на Radxa GS та камери Runcam WiFiLink v1/v2, TX-профілі й параметри."
faq:
  - q: "Що таке Adaptive-Link в OpenIPC?"
    a: "Це система, що автоматично регулює бітрейт, потужність передавача та параметри MCS/FEC залежно від якості сигналу: поблизу відео максимально чітке, на відстані якість знижується, але зв'язок залишається стабільним."
  - q: "Як встановити Adaptive-Link на наземну станцію Radxa?"
    a: "На образі v1.9.9 або новішому підключіть Radxa до домашнього Wi-Fi через nmtui і виконайте скрипт alink_install.sh з репозиторію OpenIPC/adaptive-link: спочатку gs remove, потім gs install."
  - q: "Як зрозуміти, що Adaptive-Link працює?"
    a: "Після з'єднання камери з наземною станцією на екрані з'являється інформаційне вікно alink. Якщо його немає — Adaptive-Link не працює: перевірте, що увімкнено UART0 з Mavlink та опцію Alink Drone у конфігураторі."
---
...
1. Образ v1.9.9 або новіший → вимкнути живлення Wi-Fi карток.  
